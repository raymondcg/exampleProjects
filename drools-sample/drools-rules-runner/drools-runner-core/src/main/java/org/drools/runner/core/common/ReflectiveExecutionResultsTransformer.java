...
/**
 * Default Transformer. This class will set the Reponse based upon the {@link KieQuery} annotation on a method in the
 * response.
 *
 */
public class ReflectiveExecutionResultsTransformer {

    private static final Logger logger = LoggerFactory.getLogger( ReflectiveExecutionResultsTransformer.class );
    ...
    /**
     * Produces a Reponse instance with all {@link KieQuery} annotated bean properties populated. Annotated fields in
     * the response class are hydrated via bean accessor methods. Expect a NullPointerException if Response class does
     * not expose @KieQuery fields via setter method. Any errors that may occur are caught and are only logged, no
    ...
    public static <Response> Response transform( ExecutionResults results, Class<Response> responseClazz ) {
        ...
        try {
            ...
            logger.error( String.format( "New instance of response class could not be created: %s",
